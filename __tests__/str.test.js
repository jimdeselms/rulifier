...
describe("str", () => {
    it("can understand a simple string interpolation", async () => {
        ...
        expect(await materialize(resp.value)).toBe("The value is 123.")
    })
    ...
    it("a null substitution is replaced by an empty string", async () => {
        ...
        expect(await materialize(resp.value)).toBe("No value here: ().")
    })
    ...
    it("a undefined substitution is ignored", async () => {
        ...
        expect(await materialize(resp.value)).toBe("No replacement here: (${thing}).")
    })
    ...
    it("can handle multiple replacements in the same string", async () => {
        ...
        expect(await materialize(resp.value)).toBe("Hello, Fred! Have a great Sunday!")
    })
    ...
    it("will escape substitutions that begin with a \\", async () => {
        ...
        expect(await materialize(resp.value)).toBe("${greeting}, world!")
    })
})
